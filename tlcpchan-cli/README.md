...
用户通过编辑配置文件后使用 `config reload` 重载配置。

这个设计基于以下考虑：

**为什么不提供 config update API：**

1. **原子性考虑** - 配置更新是复杂的多步骤操作（读取 → 验证 → 更新 → 写入 → 重载），通过 API 很难保证原子性和一致性
2. **数据完整性** - YAML 配置文件格式复杂，直接编辑更直观，可避免 API 部分更新导致配置损坏
3. **回滚能力** - 编辑配置文件后，用户可以手动回滚，API 模式增加了复杂性
4. **用户习惯** - 配置文件编辑是用户熟悉的运维方式
...
1. 编辑配置文件 (config.yaml)
   ↓
2. 验证配置有效性 (config validate)
   ↓
3. 重载配置 (config reload)
   ↓
4. 验证配置已生效 (config show / system info)
```
...
文件名                    大小    修改时间              当前
------                   ----     --------              ----
tlcpchan.log             1.2 MB   2024-01-01 12:00:00   *
tlcpchan.log.2024-01-02  5.8 MB   2024-01-02 00:00:00
tlcpchan.log.2024-01-01  6.2 MB   2024-01-01 00:00:00
```

**说明：**
- `当前` 列的 `*` 表示当前正在使用的日志文件
- 文件大小会自动转换为合适的单位（B, KB, MB, GB）

### 11.2 查看日志内容

读取日志文件内容，支持行数限制和日志级别过滤。

**参数说明：**

| 参数 | 说明 | 是否必需 | 默认值 |
|------|------|---------|--------|
| `--file` 或 `-f` | 日志文件名 | 否 | 当前日志文件 |
| `--lines` 或 `-n` | 读取行数（最大2000行） | 否 | 500 |
| `--level` 或 `-l` | 日志级别过滤（debug/info/warn/error） | 否 | 不过滤 |

**示例 1：查看当前日志（默认500行）**

```bash
tlcpchan-cli system logs content
```
...
总行数: 1523, 返回: 500
---
[info] 2024-01-01T12:00:00Z: 服务启动
[info] 2024-01-01T12:00:01Z: API服务监听 :20080
[info] 2024-01-01T12:00:02Z: UI服务监听 :20000
[warn] 2024-01-01T12:05:00Z: 配置文件未找到，使用默认配置
[info] 2024-01-01T12:10:00Z: 实例 my-proxy 已启动
...
```

**示例 2：查看指定日志文件（读取1000行）**

```bash
tlcpchan-cli system logs content -f tlcpchan.log.2024-01-02 -n 1000
```

**响应示例：**
```
文件: tlcpchan.log.2024-01-02
总行数: 3456, 返回: 1000
---
[error] 2024-01-02T03:15:00Z: 连接超时
[info] 2024-01-02T03:15:05Z: 重试连接...
...
```
...
```bash
tlcpchan-cli system logs content --level error
```
...
总行数: 1523, 返回: 23
---
[error] 2024-01-01T14:30:00Z: 端口 8443 已被占用
[error] 2024-01-01T15:20:00Z: 证书验证失败
[error] 2024-01-01T16:45:00Z: 连接拒绝
...
```

**说明：**
- 日志级别过滤支持：`debug`、`info`、`warn`、`error`
- 最大读取行数为2000行，超过会被自动截断
- 不指定文件名时，默认读取当前正在使用的日志文件
...
| 参数 | 说明 | 是否必需 | 默认值 |
|------|------|---------|--------|
| `<filename>` | 日志文件名（位置参数） | 是 | - |
| `--output` 或 `-o` | 输出文件路径 | 否 | 当前目录 |

**示例 1：下载日志文件到当前目录**

```bash
tlcpchan-cli system logs download tlcpchan.log.2024-01-02
```

**响应示例：**
```
已下载: tlcpchan.log.2024-01-02 -> tlcpchan.log.2024-01-02 (5789432 bytes)
```

**示例 2：下载日志文件到指定路径**

```bash
tlcpchan-cli system logs download tlcpchan.log -o /var/log/tlcpchan-debug.log
```

**响应示例：**
```
已下载: tlcpchan.log -> /var/log/tlcpchan-debug.log (1256789 bytes)
```
...
| 参数 | 说明 | 是否必需 | 默认值 |
|------|------|---------|--------|
| `--output` 或 `-o` | 输出文件路径 | 否 | 自动生成文件名 |

**示例 1：打包下载所有日志（使用默认文件名）**

```bash
tlcpchan-cli system logs download-all
```

**响应示例：**
```
已下载所有日志: tlcpchan-logs-20240101-150005.zip (13845678 bytes)
```

**示例 2：打包下载所有日志（指定文件名）**

```bash
tlcpchan-cli system logs download-all -o /backup/tlcpchan-full-logs.zip
```

**响应示例：**
```
已下载所有日志: /backup/tlcpchan-full-logs.zip (13845678 bytes)
```

**说明：**
- 不指定输出路径时，文件名格式为 `tlcpchan-logs-YYYYMMDD-HHMMSS.zip`
- ZIP 文件包含所有日志文件
- 适用于批量备份或日志分析
