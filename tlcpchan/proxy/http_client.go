...
func (p *HTTPClientProxy) Start() error {
	...
	// 启动时检测服务端协议（仅auto模式）
	if p.cfg.Protocol == string(config.ProtocolAuto) {
...
func (p *HTTPClientProxy) Reload(cfg *config.InstanceConfig) error {
	p.mu.Lock()
	oldCfg := p.cfg
	p.cfg = cfg
	p.mu.Unlock()
	...
	p.logger.Info("HTTP客户端代理配置热重载成功: %s", p.cfg.Name)
	return nil
}
...
// 返回:
//   - error: 重载失败时返回错误
func (p *HTTPClientProxy) ReloadCertificates() error {
	if err := p.adapter.ReloadCertificates(); err != nil {
		return err
	}
	p.logger.Info("HTTP客户端代理证书热重载成功: %s", p.cfg.Name)
	return nil
}
